(() => {
  ...
  function redirectLegacy() {
    const legacy = location.hash.match(/^#\/(?:post|posts|issue|issues)\/(\d+)(?:\/|$)/);
  ...
  function filter() {
    ...
    for (const card of cards) {
      card.hidden = !((!year || card.dataset.year === year) && card.dataset.search.includes(query));
  ...
  document.querySelectorAll('[data-filter-year]').forEach(link => {
    link.addEventListener('click', event => {
      event.preventDefault(); year = link.dataset.filterYear; filter();
      document.querySelector('#articles').scrollIntoView({behavior: 'auto'});
    });
  });
  clear.addEventListener('click', () => { year = ''; input.value = ''; filter(); });
})();
